...
(function() {
    ...
    function mobConvert(mob) {
        ...
        if(mob==="Slime"||mob==="Chest"||mob==="Cock") {
            return 1
        }
        ...
        if(mob==="Mojo Slime"||mob==="Big Chest"||mob==="Goomba"||mob==="Leafbug") {
            return 2
        }
        if(mob==="Bomber"||mob==="Shell") {
            return 2.2
        }
        if(mob==="Orga"||mob==="Needle"||mob==="Crab") {
            return 2.5
        }
        if(mob==="Spider") {
            return 3
        }
        if(mob==="Crystal"||mob==="Ladybug") {
            return 3.5
        }
        if(mob==="Ethereal Slime"||mob==="Bee") {
            return 3.8
        }
    ...
    function DEFcalc(mob,rar) {
        let returnValue=slotConvert(rar)*rarityATK(rar)+mobConvert(mob)*rarityFactorMob(rar)
        returnValue=Math.floor(returnValue*10+0.09)/10
        return returnValue
    }
    ...
    unsafeWindow.send=function() {
        ...
        if(newMessageValue.slice(0,5)===".pos ") {
            newMessageValue=newMessageValue.slice(6)
            unsafeWindow.choosepos(parseInt(newMessageValue,10))
            newMessageValue=""
            document.getElementById("message").value=newMessageValue
        }
